	...
	if ('00' == date('i') && (date('s') < 20)) { // top of the hour - give ~time for unstuck sensors to fully unstick?
		...
		if ($result->num_rows > 0) {
			...
			if($valid['windspeed'] || $valid['winddir']) {
				$sql = "SELECT MIN(tempf), SUM(rainin) FROM ws1400 WHERE reg_date>date_sub(now(), interval 4 hour)";
...
if (($_GET["ID"] == $wu_id) && ($_GET["PASSWORD"] == $wu_pwd) &&
	...
	if ($result->num_rows > 0) {
		...
		if ((($soilbattvolt < 3.25 ) || ($_SERVER['REQUEST_TIME'] - $soilupdatetime > 2*60*60 )) &&
			('00' == date('i') && (date('s') < 20))) {
			mail('miker_nomon@gmx.com', 'Soil Sensor Fail', "Voltage " . $soilbattvolt);
	...
	if ( $tempf > 50.0 ) {
		...
		$correction = round( $tempf*$tempf*$tempf*0.000237 + $tempf*$tempf*-0.0174 + $tempf*-0.1031 + 24.9 );
	...
	// - Weather Underground
	// example WU string
	// ID=KKSSHAWN60&PASSWORD=cg1jblt1&tempf=29.3&humidity=74&dewptf=22.1&windchillf=24.3&winddir=337&windspeedmph=4.70&windgustmph=7.61
